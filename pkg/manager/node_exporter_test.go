...
func TestNodeExporter_InvertedPolarityCondition(t *testing.T) {
	ctx := context.TODO()
	...
	conditionType := corev1.NodeConditionType("TestIssue")
	nodeExporter := manager.NewNodeExporter(
		&initialNode,
		fakeClient,
		&recorder,
		map[corev1.NodeConditionType]manager.NodeConditionConfig{
			conditionType: {
				ReadyReason:   "TestIssueIsHealthy",
				ReadyMessage:  "No test issue",
	...
	nodeKey := client.ObjectKeyFromObject(&initialNode)
	waitForCondition := func(expected corev1.NodeCondition) {
		t.Helper()
		reportChan <- time.Now()
		if err := wait.PollUntilContextTimeout(ctx, 1*time.Second, 10*time.Second, true, func(ctx context.Context) (done bool, err error) {
			var node corev1.Node
			...
			if !nodeHasCondition(node, expected) {
				t.Logf("node condition not found: %+v: %+v", expected, node.Status.Conditions)
				return false, nil
			}
			return true, nil
		}); err != nil {
	...
	// healthy state reports False instead of True
	waitForCondition(corev1.NodeCondition{
		Type:    conditionType,
		Status:  corev1.ConditionFalse,
		Reason:  "TestIssueIsHealthy",
		Message: "No test issue",
	})

	// a fatal condition flips the status to True instead of False
	if err := nodeExporter.Fatal(ctx, monitor.Condition{Reason: "TestReason", Message: "TestMessage"}, conditionType); err != nil {
		t.Fatal(err)
	}
	waitForCondition(corev1.NodeCondition{
		Type:    conditionType,
