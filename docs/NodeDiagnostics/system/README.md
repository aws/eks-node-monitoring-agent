...
- **Source:** [`pkg/log_collector/system/top.go`](../../../pkg/log_collector/system/top.go) — calls `top -b -n 1 -w 512`
- **Linux syscall:** `open(2)` on `/proc/stat`, `/proc/meminfo`, `/proc/[pid]/stat`
...
- **Command:** `ps fauxwww --headers`
- **Linux syscall:** `open(2)` on `/proc/[pid]/stat`, `/proc/[pid]/cmdline`, `/proc/[pid]/status`
...
- **Command:** `ps -eTF --headers`
- **Linux syscall:** `open(2)` on `/proc/[pid]/task/[tid]/stat`
...
- **Linux syscall:** `open(2)`, `read(2)` on `/proc/stat`
...
- **Linux syscall:** `open(2)`, `read(2)`, `getdents64(2)` (directory listing)
- **Content:** Raw stat fields for every process: PID, comm, state, ppid, pgrp, session, tty, utime, stime, vsize, rss, etc. (52 fields per the `proc(5)` man page)
...
- **Linux syscall:** `open(2)` + `read(2)` on `/sys/fs/cgroup/**/cpu.stat` and `cgroup.procs`
...
- **Linux syscall:** `open(2)` + `read(2)` on `/proc/[pid]/stat` (field 42: `blkio_ticks`)
...
- **Linux syscall:** `open(2)` + `read(2)` (file path), or `connect(2)` + `sendto(2)` (IMDS)
...
- **Linux syscall:** `connect(2)` + `sendto(2)` (HTTP to IMDS link-local address)
...
- **Linux syscall:** `connect(2)` + `sendto(2)`
- **Not collected on:** Hybrid nodes
...
- **Linux syscall:** D-Bus socket communication with `systemd` PID 1 via `connect(2)` on `/run/systemd/private/io.systemd.Manager`
...
- **Linux syscall:** `open(2)` on `/proc/net/tcp`, `/proc/net/tcp6`, `/proc/net/udp`
...
- **Command:** `rpm -qa` (RPM-based) or `deb --list` (Debian-based)
- **Linux syscall:** `open(2)` on RPM/dpkg database files
...
- **Command:** `last reboot`
- **Linux syscall:** `open(2)` on `/var/log/wtmp`
...
- **Linux syscall:** `open(2)` on `/sys/fs/selinux/enforce` (or equivalent mountpoint)
