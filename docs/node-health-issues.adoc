...
|Condition
|The number of GPUs detected by DCGM does not match the number of nvidia device files in /dev, which indicates driver issues requiring investigation.

|NvidiaDoubleBitError
|Condition
|A double bit error was produced by the GPU driver.

|NvidiaExpectedDeviceCountMismatch
|Condition
|The number of GPUs detected on the node does not match the expected count for the EC2 instance type, which indicates a GPU failed to enumerate on the PCIe bus at boot. The instance should be replaced.
